...
process_gene_data <- function(file_path, gene_name) {
  ...
  data$gene_name <- gene_name  # Add gene_name as a column
  return(data)
}

# Load data for Loss and Gain genes
# You need to load each gene separately
Loss_files <- c(
...
Loss_data <- lapply(1:length(Loss_files), function(i) {
  process_gene_data(Loss_files[i], Loss_gene_names[i])  # Pass the actual gene names
})

Gain_data <- lapply(1:length(Gain_files), function(i) {
  process_gene_data(Gain_files[i], Gain_gene_names[i])  # Pass the actual gene names
})
...
# Step 2: Generate contrasting colors for these nodes (still 5 colors in this example)
# Note: 'n' should match the number of nodes you want to color (5 in your case)
...
pdf("/Users/lfitzger/Desktop/PhD/Chapter2_ClownfishStripes/Plots/Loss_Genes.pdf", width = 14, height = 8)
...
pdf("/Users/lfitzger/Desktop/PhD/Chapter2_ClownfishStripes/Plots/Gain_Genes.pdf", width = 14, height = 8)
